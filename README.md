...
<!-- This isn't needed at all, it's already in the index, or it needs to be removed from the supplied index file -->
*index.html*
```html
<form id="newTaskForm">
...
Finally, let's make it so that we can open and close the new task form with the add-todo and Cancel button.
...
$(document).on('click', '#item', function(e) {
  ...
  advanceTask(task);
  this.id = 'inProgress';
  $('#inProgress').append(this.outerHTML);
});
```
